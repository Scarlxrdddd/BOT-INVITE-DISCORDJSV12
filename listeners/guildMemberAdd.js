...
module.exports = class GuildMemberAddListener extends Listener {
    ...
    async exec(member) {
        ...
        if(member.user.bot) return welcomeChannel.send(`${member.toString()} a rejoint le serveur.`).catch(err => console.log(err));
        ...
        if(usedInvite.code === member.guild.vanityURLCode) {
            welcomeChannel.send(`${member.toString()} a rejoint le serveur en utilisant l'url personnalisée !`);
        ...
        if(!usedInvite) return welcomeChannel.send(`${member.toString()} a rejoint le serveur mais je ne sais pas comment. Peut être une invitation personnalisée ?`).catch(err => console.log(err));
